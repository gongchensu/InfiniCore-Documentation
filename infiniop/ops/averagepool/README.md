...
`AveragePool`, 即**平均池化**算子，对输入张量进行平均池化操作，通过滑动窗口计算局部区域的平均值来降低空间维度。

对于输入张量 $x$，在空间维度上应用平均池化，输出张量 $y$ 的计算公式为：

$$ y_{b,c,i_1,...,i_k} = \frac{1}{|\Omega|} \sum_{(j_1,...,j_k) \in \Omega} x_{b,c,j_1,...,j_k} $$

其中 $\Omega$ 是池化窗口内的有效位置集合，$|\Omega|$ 是有效位置的数量。
...
- `desc`: 已使用 `infiniopCreateAvgPoolDescriptor()` 初始化的算子描述符；
